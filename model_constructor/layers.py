...
__all__ = ['ConvLayer', 'act_fn', 'Flatten', 'Noop', 'DownsampleBlock', 'BasicBlock', 'Bottleneck', 'DownsampleLayer',
           'XResBlock', 'conv1d', 'SimpleSelfAttention', 'ConvBlockBasic', 'ConvBlockBottle', 'noop', 'ResBlock']
...
class ResBlock(nn.Module):
    def __init__(self, ni, nh, expansion=1, stride=1, conv_layer=ConvLayer,
                 ...
                 act_fn=act_fn, zero_bn=True, sa=False, sym=False, **kwargs):
        super().__init__()
        nf,ni = nh*expansion,ni*expansion
        self.convs = conv_block(conv_layer,ni,nh,nf,stride,zero_bn,**kwargs)
        self.sa = SimpleSelfAttention(nf,ks=1,sym=sym) if sa else noop
        self.identity = downsample(conv_layer, ni, nf, stride, act=False, **kwargs) if ni!=nf or stride==2 else Noop()
        self.merge = Noop() # dummy layer for visualize  residual connection
        self.act_fn = act_fn
    def forward(self, x): return self.act_fn(self.merge(self.sa(self.convs(x)) + self.identity(x)))