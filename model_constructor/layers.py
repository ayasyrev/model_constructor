...
__all__ = ['ConvLayer', 'act_fn', 'Flatten', 'noop', 'Noop', 'Mish', 'DownsampleBlock', 'BasicBlock', 'Bottleneck',
           'DownsampleLayer', 'XResBlock', 'conv1d', 'SimpleSelfAttention', 'SEBlock', 'SEBlockConv', 'ConvBlockBasic',
           'ConvBlockBottle', 'ResBlock']
...
#         c_in = math.ceil(c//r/8)*8
        ...
        self.excitation = nn.Sequential(OrderedDict([
            ('conv_reduce', self.excit_layer(c, c_in, 1, bias=False)),
            ('se_act', self.act_fn),
            ('conv_expand', self.excit_layer(c_in, c, 1, bias=False)),
            ('sigmoid', nn.Sigmoid())
        ]))
