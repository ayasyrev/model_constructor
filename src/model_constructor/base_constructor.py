        ...
        stem = [
            (
                ...
                conv_layer(
                    self.sizes[i],
                    self.sizes[i + 1],
                    stride=2 if i == stride_on else 1,
                    act=True,
        ...
        **kwargs,
        ...
        self.conv = nn.Sequential(
            OrderedDict(
                [
                    (
                        "conv_0",
                        conv_layer(ni, nf, stride=stride, act_fn=act_fn, **kwargs),
                    ),
                    (
                        "conv_1",
                        conv_layer(
                            nf, nf, zero_bn=zero_bn, act=False, act_fn=act_fn, **kwargs
                        ),
                    ),
                ]
            )
        ...
        if self.downsample:
            self.downsample = downsample_block(
                conv_layer, ni, nf, ks=1, stride=stride, act=False, **kwargs
            )
        self.merge = Noop()
        ...
        self.conv = nn.Sequential(
            OrderedDict(
                [
                    ...
                    (
                        "conv_1",
                        conv_layer(nh, nh, stride=stride, act_fn=act_fn, **kwargs),
                    ),
                    (
                        "conv_2",
                        conv_layer(
...
class DownsampleLayer(nn.Sequential):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        layers += (
            [] if ni == nf else [("idconv", conv_layer(ni, nf, 1, act=act, **kwargs))]
        )
        if not pool_1st:
            layers.reverse()
        super().__init__(OrderedDict(layers))
        ...
        **kwargs,
        ...
        layers = (
            [
                (
                    "conv_0",
                    conv_layer(ni, nh, 3, stride=stride, act_fn=act_fn, **kwargs),
                ),
                (
                    "conv_1",
                    conv_layer(
                        nh, nf, 3, zero_bn=zero_bn, act=False, act_fn=act_fn, **kwargs
                    ),
                ),
            ]
            if expansion == 1
                ...
                (
                    "conv_1",
                    conv_layer(nh, nh, 3, stride=stride, act_fn=act_fn, **kwargs),
                ),
                (
                    "conv_2",
                    conv_layer(
                        nh, nf, 1, zero_bn=zero_bn, act=False, act_fn=act_fn, **kwargs
                    ),
                ),
            ]
        )
        ...
        self.identity = (
            DownsampleLayer(
                conv_layer, ni, nf, stride, act=False, act_fn=act_fn, **kwargs
            )
            if ni != nf or stride == 2
            else Noop()
        )
        self.merge = Noop()
        self.act_fn = act_fn
...
def xresnet18(**kwargs):
    ...
    return Net(
        stem_sizes=[32, 32], block=XResBlock, blocks=[2, 2, 2, 2], expansion=1, **kwargs
    )
...
def xresnet34(**kwargs):
    ...
    return Net(
        stem_sizes=[32, 32], block=XResBlock, blocks=[3, 4, 6, 3], expansion=1, **kwargs
    )
...
def xresnet50(**kwargs):
    ...
    return Net(
        stem_sizes=[32, 32], block=XResBlock, blocks=[3, 4, 6, 3], expansion=4, **kwargs
    )
