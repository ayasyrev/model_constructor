...
class ConvTwist(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.twist:
            ...
            self.coeff_Ax = nn.Parameter(
                torch.empty((nf, ni // groups)).normal_(0, std), requires_grad=True
            )
            self.coeff_Ay = nn.Parameter(
                torch.empty((nf, ni // groups)).normal_(0, std), requires_grad=True
            )
        self.iters = iters
        self.stride = stride
    ...
    def full_kernel(self, kernel):  # permuting the groups
        ...
        for i in range(n):
            if i % 4 == 0:
                KK[a * i : a * (i + 1), b * (i + 3) : b * (i + 4)] = kernel[
                    a * i : a * (i + 1)
                ]
            else:
                KK[a * i : a * (i + 1), b * (i - 1) : b * i] = kernel[
                    a * i : a * (i + 1)
                ]
        return KK
    ...
    def _conv(self, inpt, kernel=None):
        ...
        if self.permute is False:
            return F.conv2d(
                inpt, kernel, padding=1, stride=self.stride, groups=self.groups
            )
        else:
            return F.conv2d(
                inpt, self.full_kernel(kernel), padding=1, stride=self.stride, groups=1
            )
        ...
        if self.same:
            ...
            for i in range(self.groups):
                conv_wt.data[n * i : n * (i + 1)] = (
                    conv_wt[n * i : n * (i + 1)]
                    + torch.transpose(conv_wt[n * i : n * (i + 1)], 0, 1)
                ) / 2
    ...
    def forward(self, inpt):
        ...
        _, _, h, w = out.size()
        XX = (
            torch.from_numpy(np.indices((1, 1, h, w))[3] * 2 / w - 1)
            .type(out.dtype)
            .to(out.device)
        )
        YY = (
            torch.from_numpy(np.indices((1, 1, h, w))[2] * 2 / h - 1)
            .type(out.dtype)
            .to(out.device)
        )
        kernel_x = self.kernel(self.coeff_Ax, self.coeff_Ay)
        self.symmetrize(kernel_x)
        kernel_y = kernel_x.transpose(2, 3).flip(
            3
        )  # make conv_y a 90 degree rotation of conv_x
        ...
        if self.same and self.iters > 1:
            ...
            for _ in range(self.iters - 1):
                out = (
                    ...
                    + (
                        self._conv(out)
                        + XX * self._conv(out, kernel_x)
                        + YY * self._conv(out, kernel_y)
                    )
                    / self.iters
                )
            out = out - inpt
...
class NewResBlockTwist(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        layers = (
            [
                ("conv_0", conv_layer(ni, nh, 3, act_fn=act_fn, bn_1st=bn_1st)),
                (
                    "conv_1",
                    conv_layer(nh, nf, 3, zero_bn=zero_bn, act=False, bn_1st=bn_1st),
                ),
            ]
            if expansion == 1
            else [
                ...
                (
                    "conv_1_twist",
                    ConvLayerTwist(nh, nh, 3, act_fn=act_fn, bn_1st=bn_1st),
                ),
                (
                    "conv_2",
                    conv_layer(nh, nf, 1, zero_bn=zero_bn, act=False, bn_1st=bn_1st),
                ),
            ]
        )
        if sa:
        ...
        self.idconv = (
            noop if ni == nf else conv_layer(ni, nf, 1, act=False, bn_1st=bn_1st)
        )
        self.merge = act_fn
        ...
        **kwargs,
        ...
        layers = (
            [
                (
                    "conv_0",
                    conv_layer(ni, nh, 3, stride=stride, act_fn=act_fn, bn_1st=bn_1st),
                ),
                (
                    "conv_1",
                    conv_layer(nh, nf, 3, zero_bn=zero_bn, act=False, bn_1st=bn_1st),
            ...
            if expansion == 1
                ...
                (
                    "conv_1_twist",
                    ConvLayerTwist(
                        nh, nh, 3, stride=stride, act_fn=act_fn, bn_1st=bn_1st
                    ),
                ),
                (
                    "conv_2",
